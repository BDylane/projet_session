...
def update_order(order_id):
    ...
    if not data and ("order" not in data or "credit_card" not in data):
    ...
    if "credit_card" in data:
        ...
        payload = {
            ...
            "amount_charged": int(round(order.total_price + order.shipping_price))  # total incluant shipping
        }
        payloadTest = {
            "credit_card": {
                "name": "John Doe",
        ...
        try:
            # response = requests.post(payment_url, json=payloadTest)    Temporairement on utilisera un mock car pas moyen de faire fonctionner la vraie requete
